...
function readConfigFile(file) {
  ...
  if (bytes[0] === 0xef && bytes[1] === 0xbb && bytes[2] === 0xbf) {
    return { text: '', config: {}, error: { kind: 'bom', line: 1, column: 1,
      message: 'config.json starts with an invisible mark that Notepad and PowerShell add when they save. In Notepad choose Save as and pick UTF-8 without that mark, or run  python install.py  to write it fresh.' } };
  }
...
function refusalLines(error, file) {
  return [
    ...
    'Fix that line, or run  python install.py  to write the file again. Your file was not touched.',
  ];
}
...
module.exports = { readConfigFile, lineAndColumn, plainReason, lastGoodPort, refusalLines, damagedLines };
