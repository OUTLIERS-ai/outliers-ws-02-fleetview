...
//  - "waiting for you" lasts until the member answers (upstream: 15 seconds).
...
//  - a big log file is read in pieces of at most 16 MB, never as 1 string
'use strict';
const fs = require('fs');
...
const DEFAULT_WAIT_CAP_H = 8;          // a wait older than this shows as idle and leaves the Needs you strip

function isInterrupt(text) { return /^\[Request interrupted by user/.test(String(text || '')); }

function createStore(opts = {}) {
  const now = opts.now || (() => Date.now());
  const chunkBytes = opts.chunkBytes || 16 * 1024 * 1024;
  ...
  function getSession(id) {
    ...
    if (!s) {
      s = {
        ...
        mainAt: null, mainType: '', mainStop: null, mainTypes: [], mainInterrupt: false,
  ...
  function processEvent(event, meta = {}) {
    ...
    if (!isSub && (event.type === 'assistant' || (event.type === 'user' && !event.isMeta)) && (!s.mainAt || ts >= s.mainAt)) {
      ...
      const firstText = typeof content === 'string' ? content : (Array.isArray(content) ? (content.find(c => c && c.type === 'text') || {}).text : '');
      s.mainInterrupt = event.type === 'user' && isInterrupt(firstText);
    }
  ...
  // tests and the first start-up scan are predictable. Reads in pieces of at most
  // chunkBytes, so a log of any size never has to fit in 1 string (Node refuses
  // strings above about 512 MB).
  function processFile(filePath) {
    ...
    try {
      ...
      for (;;) {
        ...
        const len = Math.min(stat.size - start, chunkBytes);
        ...
        if (lastNl < 0) {
          if (start + len >= stat.size) return; // no complete line yet; try again next change
          // 1 line longer than a whole piece: skip past it rather than grow without limit
          const nl = findNextNewline(filePath, start + len, stat.size);
          offsets.set(filePath, nl < 0 ? stat.size : nl + 1);
          continue;
        }
  ...
  // working ("thinking"), waiting for the member, or idle.
  //  - the member's message (or a tool result) was the last word: working, for up to 10 minutes
  //  - the member pressed Esc: idle
  //  - Claude's finished reply was the last word: waiting, until the member answers
  //    (for up to waiting_hours, 8 by default; after that it shows as idle and leaves the Needs you strip)
  //  - Claude asked to run a tool and nothing came back for 30 seconds: waiting, reason "tool"
  //    (it may need the member's approval, or a long step is running)
  //  - any helper agent (subagent) active in the last 15 seconds: working
  function statusOf(s) {
    const t = now();
    ...
    if (tool) {
      ...
      return age < waitCapMs ? { status: 'waiting', waitReason: 'tool', waitingSince: s.mainAt } : { status: 'idle' };
    }
    const finished = FINAL_STOPS.has(s.mainStop) || (s.mainStop === null && s.mainTypes.includes('text'));
    if (finished) return age < waitCapMs ? { status: 'waiting', waitReason: 'reply', waitingSince: s.mainAt } : { status: 'idle' };
  ...
  function view(s) {
    ...
    return {
      ...
      priceKnown: !!getPricing(s.model), waitReason: null, waitingSince: null, ...statusOf(s),
...
module.exports = { createStore, lastPart, DEFAULT_WAIT_CAP_H };
