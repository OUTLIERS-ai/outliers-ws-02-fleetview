...
- **You are screen-sharing.** The side panel shows folder paths, file names and the start of what you typed. Set `hide_paths` to `true` in FleetView's settings file `config.json`, then run `python install.py --stop` and `python install.py --start`, before you share: the change only takes effect after that restart.
...
- **claude-view** was ruled out because it runs on macOS only. 2 other tools were ruled out because they needed Docker (a tool for running programs in a sealed box) or Bash (the Mac and Linux command line), which are awkward on Windows.
...
![Budgets set, 2026-06-12 22:45: the 5-hour window in red at 197.65M tokens used of the 200.00M budget Ashley typed in, 209.28M projected by the end, 15 minutes until it resets; then the 7-day total, 976.71M tokens of a 1.50B budget. M means million, B means billion.](img/history-2026-06-12-budgets.png)
...
If you would rather have a bigger, polished tool that someone else maintains, look at **Claude-Code-Agent-Monitor** (https://github.com/hoangsonww/Claude-Code-Agent-Monitor, MIT licence: free to use and change, as long as the credit stays). On 2026-09-22 it had 1,012 stars and a change the day before. It is a full app you open at http://localhost:4820, with a database, charts of how your agents work, a Windows and Mac desktop app that sits beside the clock, and an interface you can read in 5 languages.
...
5. It asks where your **second brain folder** is, then your **CRM folder**. (If you keep them in Obsidian, the note-taking app, these are what Obsidian calls vaults.) It looks in your Documents folder and your home folder and suggests the likeliest folder in brackets. Press Enter to accept, type a different path, or type `-` to skip.
...
| `projects_dir` | The folder Claude Code writes its session logs into. Leave it out unless Claude Code keeps its logs somewhere other than `.claude\projects` in your home folder |
...
![What the page looks like when FleetView has stopped: a red banner with the command to start it again, and a top bar that has gone quiet with it: the count reads "2 needed you at 12:09" instead of pretending to be live. Made-up data.](img/fleetview-stopped.png)

![The top of the page when `config.json` cannot be read: an amber banner naming the fault, saying that until the file is fixed your sessions will not be grouped under the folder names you chose, and naming the port FleetView kept so the browser tab you already have open still works. Made-up data.](img/config-broken.png)
